...
#define fxprint(a) 	printFix(expand(a));
...
int32_t roundcon (uint32_t i) {
	// Equivalent to floor((i* 512/360)+0.5)
	return ( (((i*512) << FIX14_SHIFT))/360 + 8192 ) >> FIX14_SHIFT;
}

int32_t calcSin(int32_t i){
	//find equivalent angle in [-359,359]
	if( (359< i) + (i <-359)) i%=360;

	//find equivalent angle in [0,359]
	if(i<0) i+=360;

	//return the SIN to the angle
	return SIN[roundcon(i)];
}

int32_t calcCos(int32_t i) {
	// cos(v) = sin(v+90 degrees)
	return calcSin(i + 90);
}

void initVector(vector_t *v, int32_t nx, int32_t ny) {
	v->x = nx;
	v->y = ny;
}
...
void rotateVector(vector_t * v, int32_t vinkel) {
	...
	// x' = x*cos(v) - y*sin(v
	v->x = tx*calcCos(vinkel) - ty*calcSin(vinkel);

	// y' = x*sin(v) + y*cos(v)
	v->y = tx*calcSin(vinkel) + ty*calcCos(vinkel);
}
